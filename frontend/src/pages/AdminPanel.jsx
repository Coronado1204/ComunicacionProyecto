...
export const AdminPanel = () => {
  ...
  const { data: reportsData, isLoading: loadingReports } = useQuery({
    ...
    queryFn: async () => { const { data } = await api.get('/reports?limit=100'); return data.data }
  })
    ...
    <main className="space-y-6" aria-labelledby="admin-title">
      ...
      <section className="rounded-2xl p-6 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E3A8A 50%, #2563EB 100%)' }}>
        ...
        <div className="relative flex items-center gap-3">
          <div className="w-10 h-10 bg-white/10 backdrop-blur-sm rounded-xl flex items-center justify-center border border-white/20" aria-hidden="true">
            <Shield size={20} className="text-white" />
          </div>
          <div>
            <h1 id="admin-title" className="text-2xl font-bold text-white">Panel de Administrador</h1>
            <p className="text-blue-200 text-sm">Gestión completa del sistema</p>
          </div>
        </div>
      </section>
      ...
      <section aria-label="Estadísticas del sistema">
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Usuarios registrados', value: totalUsuarios, color: 'from-brand-500 to-brand-700',  text: 'text-brand-600' },
            { label: 'Reportes totales',      value: totalReportes, color: 'from-blue-400 to-blue-600',    text: 'text-blue-600'  },
            { label: 'Reportes resueltos',    value: resueltos,     color: 'from-green-400 to-green-600',  text: 'text-green-600' },
          ].map(({ label, value, color, text }) => (
            <article key={label} className="card flex flex-col gap-3" role="status" aria-label={label + ': ' + value}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-neutral-500 uppercase tracking-wide">{label}</span>
                <div className={'w-8 h-8 rounded-xl flex items-center justify-center bg-gradient-to-br ' + color} aria-hidden="true">
        ...
        {[
          ...
          <button key={id} role="tab" aria-selected={tab === id}
            ...
            className={
              ...
              (tab === id ? 'border-brand-600 text-brand-600' : 'border-transparent text-neutral-400 hover:text-neutral-700')
            }
          >
      ...
      {/* Panel Usuarios */}
      <section id="panel-usuarios" role="tabpanel" aria-labelledby="tab-usuarios"
        hidden={tab !== 'usuarios'} className="space-y-3">
        {loadingUsers ? <SkeletonTable rows={3} /> : users?.length === 0 ? (
          ...
          <div role="list" aria-label="Lista de usuarios">
            {users?.map(u => (
              <div key={u.id} role="listitem" className="card flex items-center justify-between gap-4 mb-3 hover:border-brand-200 transition-all">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 bg-gradient-to-br from-brand-500 to-brand-700 rounded-full flex items-center justify-center shrink-0" aria-hidden="true">
                    <span className="text-sm font-bold text-white">{u.name?.charAt(0).toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span className={'badge text-xs ' + roleColor(u.role)} aria-label={'Rol: ' + u.role}>{u.role}</span>
                      {!u.active && <span className="badge bg-red-50 text-red-600 text-xs">Inactivo</span>}
                    </div>
                    <p className="font-bold text-neutral-900">{u.name}</p>
                    <p className="text-sm text-neutral-500">{u.email} · {u.municipio}</p>
                    <p className="text-xs text-neutral-400 mt-0.5">{u._count.reports} reportes · Desde {new Date(u.createdAt).toLocaleDateString('es-CO')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <label htmlFor={'role-' + u.id} className="sr-only">Cambiar rol de {u.name}</label>
                  <select id={'role-' + u.id} className="input w-36 text-sm" value={u.role}
                    onChange={e => roleMutation.mutate({ id: u.id, role: e.target.value })}
                    disabled={u.id === user?.id} aria-disabled={u.id === user?.id}>
                    {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                  </select>
                  <button onClick={() => toggleMutation.mutate(u.id)} disabled={u.id === user?.id}
                    aria-label={(u.active ? 'Desactivar' : 'Activar') + ' usuario ' + u.name}
                    aria-pressed={u.active}
                    className="p-2 rounded-xl hover:bg-neutral-100 transition-colors disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2">
                    {u.active
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl hover:bg-red-50 text-neutral-400 hover:text-red-500 transition-colors disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2">
      ...
      <section id="panel-reportes" role="tabpanel" aria-labelledby="tab-reportes"
        hidden={tab !== 'reportes'} className="space-y-3">
        {loadingReports ? <SkeletonTable rows={3} /> : reports.length === 0 ? (
          ...
          <div role="list" aria-label="Lista de reportes">
            {reports.map(r => (
              <div key={r.id} role="listitem" className="card flex items-start justify-between gap-4 mb-3 hover:border-brand-200 transition-all">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={'badge text-xs ' + statusColor(r.status)} aria-label={'Estado: ' + r.status}>{r.status}</span>
                    <span className="badge bg-neutral-100 text-neutral-600 text-xs">{r.category}</span>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <select id={'status-' + r.id} className="input w-36 text-sm" value={r.status}
                    onChange={e => statusMutation.mutate({ id: r.id, status: e.target.value })}>
                    {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <button
                    onClick={() => { if (confirm('¿Eliminar "' + r.title + '"?')) deleteReportMutation.mutate(r.id) }}
                    aria-label={'Eliminar reporte ' + r.title}
                    className="p-2 rounded-xl hover:bg-red-50 text-neutral-400 hover:text-red-500 transition-colors focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2">
                    <Trash2 size={16} aria-hidden="true" />
                  </button>
                </div>
              </div>
