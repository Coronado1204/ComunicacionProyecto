...
const StatCard = ({ icon: Icon, label, value, sub, color }) => (
  ...
  >
    ...
    <div>
      <p className="text-3xl font-semibold text-neutral-900" aria-live="polite">
        {value ?? '—'}
      </p>
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-white border border-neutral-100 rounded-xl px-3 py-2 text-sm" role="tooltip">
...
export const Dashboard = () => {
  ...
  return (
    <main className="space-y-8" aria-labelledby="dashboard-title">

      {/* Header */}
      <section aria-label="Bienvenida">
        <h1 id="dashboard-title" className="text-2xl font-semibold text-neutral-900">
      ...
      <section aria-label="Estadísticas generales">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={FileText}    label="Total reportes" value={total}     sub="En el sistema"          color="bg-neutral-100 text-neutral-600" />
            <StatCard icon={AlertCircle} label="Pendientes"      value={pendientes} sub="Requieren atención"    color="bg-amber-50 text-amber-500" />
            <StatCard icon={Clock}       label="En proceso"      value={enProceso}  sub="En gestión"            color="bg-blue-50 text-blue-500" />
            <StatCard icon={CheckCircle} label="Resueltos"       value={resueltos}  sub={total > 0 ? Math.round((resueltos/total)*100) + '% del total' : '0%'} color="bg-green-50 text-green-500" />
          </div>
        )}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        {/* Chart */}
        <section className="card lg:col-span-3" aria-label="Gráfica de reportes por categoría">
          <div className="flex items-center justify-between mb-6">
            ...
            <TrendingUp size={16} className="text-neutral-300" aria-hidden="true" />
          </div>
          {isLoading ? (
            <div className="h-48 flex items-center justify-center" role="status" aria-label="Cargando gráfica">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart
                ...
                role="img"
                aria-label="Gráfica de barras con reportes por categoría"
              >
            ...
            <div className="flex items-center justify-center h-48 text-neutral-300" role="status">
        ...
        {/* Recent Reports */}
        <section className="card lg:col-span-2" aria-label="Últimos reportes">
          <div className="flex items-center justify-between mb-6">
            <div>
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-32 text-neutral-300" role="status">
              <p className="text-sm">Sin reportes aún</p>
            </div>
          ) : (
            <ol aria-label="Lista de últimos reportes">
              {reports.slice(0, 5).map(r => (
                <li key={r.id} className="flex items-start gap-3 mb-4 last:mb-0">
                  <div
                    className="w-8 h-8 bg-neutral-100 rounded-lg flex items-center justify-center shrink-0 mt-0.5"
                    aria-hidden="true"
                  >
                  ...
                  <span
                    className={'badge shrink-0 ' + (STATUS_COLORS[r.status] || 'bg-neutral-100 text-neutral-500')}
                    aria-label={'Estado: ' + (STATUS_LABELS[r.status] || r.status)}
                  >
